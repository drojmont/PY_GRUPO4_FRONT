...
const Navbar = () => {
    ...
    useEffect(() => {
        if (localStorage.getItem("userGoTrip")) {
            const user = JSON.parse(localStorage.getItem("userGoTrip"));
            setUserLogin(user);
        }
    }, []);
    ...
    return (
        ...
        {userLogin ? (
            <div className="flex items-center space-x-4">
                <span className="text-white font-semibold"><HiOutlineUser size={25} /></span>
                <span className="text-white font-semibold">Hola, {userLogin.user.nombre} {userLogin.user.apellido}</span>
                <div className="relative">
                    <button className="bg-gray-200 text-gray-800 font-bold w-10 h-10 flex items-center justify-center rounded-full hover:bg-gray-300 transition"
                        onClick={() => setDropdownOpen(!dropdownOpen)} >
                        {userLogin?.user?.nombre?.charAt(0)}
                    </button>
                    {dropdownOpen && (
                        <div className="absolute right-0 mt-2 w-48 bg-white shadow-lg rounded-lg py-2">
                            <button className="flex block w-full text-left px-4 py-2 text-red-600 hover:bg-red-100" onClick={handleLogout} >
                            <HiOutlineLogout size={25} /> <span className="pl-2">Cerrar Sesión</span>
                            </button>
                        </div>
                    )}
                </div>
